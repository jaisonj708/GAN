...
from tensorflow.keras.layers import Dense, Flatten, Conv2D, BatchNormalization, LeakyReLU, Reshape, Conv2DTranspose, ReLU, GaussianNoise
...
class Generator_Model(tf.keras.Model):
    def __init__(self):
        ...
        m.add(Dense(4*4*512, input_shape=(args.z_dim,)))
        m.add(LeakyReLU())
        m.add(Reshape((4,4,512)))
        for num in self.channels[1:-1]:
            m.add(Conv2DTranspose(filters=num, kernel_size=(5,5), strides=(2,2), padding='same'))
            m.add(LeakyReLU())
...
class Discriminator_Model(tf.keras.Model):
    def __init__(self):
        ...
        m.add(GaussianNoise(stddev=0.1,input_shape=(64,64,self.channels[0])))
    ...
    def loss_function(self, disc_real_output, disc_fake_output):
        ...
        """
        reals = disc_real_output
        fakes = disc_fake_output
        ret = tf.keras.losses.binary_crossentropy(y_pred = reals, y_true = tf.ones_like(reals))
        ret += tf.keras.losses.binary_crossentropy(y_pred = fakes, y_true = tf.zeros_like(fakes))
...
def train(generator, discriminator, dataset_iterator, manager):
    ...
    for iteration, batch in enumerate(dataset_iterator):
        with tf.GradientTape(persistent=True) as tape:
            noise = tf.random.uniform(shape=[args.batch_size,args.z_dim], minval=-1, maxval=1)
        ...
        if iteration % args.num_gen_updates == 0:
            grads = tape.gradient(d_loss, discriminator.model.trainable_variables)
        ...
        if iteration % 500 == 0:
            fid_ = fid_function(batch, fake_imgs)
            fid_total += fid_
            count += 1
            print('**** INCEPTION DISTANCE: %g ****' % fid_)

    return fid_total/count
...
def test(generator):
    ...
    num = 10
    noise = tf.random.uniform(shape=[num,args.z_dim], minval=-1, maxval=1)
    img = generator(noise)
